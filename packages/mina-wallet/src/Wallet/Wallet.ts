import { AccountInfo, Mina, SubmitTxArgs, SubmitTxResult, TransactionsByAddressesArgs } from '@palladxyz/mina-core'
import { AccountInfoGraphQLProvider, ChainHistoryGraphQLProvider, TxSubmitGraphQLProvider } from '@palladxyz/mina-graphql'
import { ConstructedTransaction, constructTransaction, SignedTransaction, signTransaction } from '@palladxyz/tx-construction'
import { VaultStore, PublicCredential } from '@palladxyz/vault'
import { Network, KeyGeneratorFactory, generateMnemonic, wordlist } from '@palladxyz/key-generator'
...
export class MinaWalletImpl implements MinaWallet {
  ...
  async submitTx(submitTxArgs: SubmitTxArgs): Promise<SubmitTxResult> {
    ...
    async createWallet(walletName: string, network: Network, accountNumber: number): Promise<{ publicKey: string; mnemonic: string } | null> {
        const mnemonic = generateMnemonic(wordlist);
        const keyGenerator = KeyGeneratorFactory.create(network);
        const keypair = await keyGenerator.deriveKeyPair({mnemonic, accountNumber});
        ...
        this.vaultStore.setCurrentWalletPublicKey({ currentWalletPublicKey: keypair.publicKey });
    
        // Return the public key and mnemonic
        return { publicKey: keypair.publicKey, mnemonic };
      }
  
      async restoreWallet(walletName: string, network: Network, mnemonic: string, accountNumber: number): Promise<{ publicKey: string } | null> {
        const keyGenerator = KeyGeneratorFactory.create(network);
        const keypair = await keyGenerator.deriveKeyPair({mnemonic, accountNumber});
      ...
      getAccounts(): string[] {
        ...
        return credentials.map((credential: { walletPublicKey: any }) => credential.walletPublicKey);
      }
