...
export const Autocomplete = React.forwardRef<
  AutocompleteRef,
  AutocompleteProps
>(({ value, onBlur, onFocus, setValue, options, onChange, ...rest }, ref) => {
  ...
  const extendedFocus: React.FocusEventHandler<HTMLInputElement> = (event) => {
    onFocus?.(event)
    setOpen(true)
    return event
  }
      ...
      {open && internalValue.length > 0 && filteredOptions.length > 0 && (
        ...
        >
          {filteredOptions.map((option: string) => (
            <CommandItem key={option} onSelect={extendedSelect} value={option}>
              {option}
            </CommandItem>
          ))}
        </CommandList>
